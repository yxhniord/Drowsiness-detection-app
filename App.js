...
export default function App() {
  ...
  const [isYawning, setIsYawning] = useState(null);
  ...
  useEffect(() => {
    async function prepare() {
      ...
      const modelWeights = require('./assets/models_new/weights.bin');
  ...
  const handleCameraStream = (images, updatePreview, gl) => {
    ...
    const loop = () => {
      ...
      tf.tidy(() => {
        ...
        if (logits) {
          setIsYawning(logits[0] > logits[1]);
        } else {
          setIsYawning(null);
        }
